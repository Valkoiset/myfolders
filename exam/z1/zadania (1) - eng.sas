...
*assign library;
libname zab "/folders/myfolders/exam";

/* proc print data=zab.z1; */
/* run; */
...
*task 1. doesn't work with where condition;
/* proc standard data = zab.z1 mean = 0 std = 1 out = z1 (where = (val gt 3 or val lt -3)); */
/* run; */
*works without it though;
proc standard data = zab.z1 mean = 0 std = 1 out = z1;
run;
*Answer: there are 3 observations (Odp: S� 3 obserwacje odstaj�ce.);
